...
def get_game_date_links(yr='2018'):
    base_url = 'http://www.ncaa.com'
    url = '/'.join((base_url,'scoreboard','lacrosse-women','d1',yr,'11','28'))
    page = urlopen(url)
...
def get_game_links(game_date_link):
    soup = BeautifulSoup(urlopen(game_date_link))
...
def get_data(game_link):
    ...
    for tab in game_data['tabsArray'][0]:
        if tab['type'] == 'play-by-play':
            ...
            pbp_data = json.load(urlopen(pbp_url))
